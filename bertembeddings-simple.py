...
def compute_input_arrays_simple(df, columns, tokenizer):
    """
    inputs:
    ...
    # create a list populated with empty lists for the maximum number of documents your data has,
    # times 3 since each document has three outputs after being run through the BERT tokenizer ([input_ids, input_masks, input_segments])
    model_input = [[]] * ((len(df)+1)*3) # total # of inputs = (# of sentences + doc) * 3 different inputs for bert)
...
np.save('inputs-simple-100k.npy', inputs)
